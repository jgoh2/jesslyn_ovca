...
#compare ranks 
ggplot(both, aes(x = data.rank, y = scale_data.rank)) + geom_point(aes(alpha = 0.5, colour = factor(both.DE))) + labs(title = "Comparing gene ranks when using data vs. scale.data", x = "data", y = "scale.data", colour = "DE for both slots", alpha = "Transparency") + stat_cor(method = "pearson")
ggsave("DF20_FindMarkers_slot_ranks.png", path = "jesslyn_plots/PDX_test")
```
...
```{r FindMarkers data slot padj, warning = FALSE, message=FALSE, fig.align='center'}
#compare padj 
ggscatter(both, x = "p_val_adj.x", y = "p_val_adj.y", add = "reg.line", add.params = list(color = "grey")) + labs(title = "Comparing the padj values of using data vs. scale.data", x = "data", y = "scale.data") + stat_cor(method = "pearson")
ggsave("DF20_FindMarkers_slot_padj.png", path = "jesslyn_plots/PDX_test")


#compare avg_logfc vs. avg_diff
ggplot(both, aes(x = avg_logFC, y = avg_diff)) + geom_point(aes(alpha = 0.5, colour = factor(both.DE))) + labs(title = "Comparing the logFC and avg_diff values of using data vs. scale.data", x = "data", y = "scale.data", colour = "DE for both slots", alpha = "Transparency")
ggsave("DF20_FindMarkers_slot_logFC.png", path = "jesslyn_plots/PDX_test")
```
...
#compare padj 
ggpairs(all, columns = c("wilcox.padj", "t.padj", "MAST.padj", "LR.padj"), diag = "blank") + labs(title = "Comparing padj values between statistical tests")
ggsave("DF20_FindMarkers_test_padj.png", path = "jesslyn_plots/PDX_test")

#compare logFC
ggpairs(all, columns = c("wilcox.logFC", "t.logFC", "MAST.logFC", "LR.logFC"), diag = "blank") + labs(title = "Comparing logFC values between statistical tests")
ggsave("DF20_FindMarkers_test_logFC.png", path = "jesslyn_plots/PDX_test")

#compare ranks
ggpairs(all, columns = c("wilcox.rank", "t.rank", "MAST.rank", "LR.rank"), diag = "blank") + labs(title = "Comparing gene ranks between statistical tests")
ggsave("DF20_FindMarkers_test_ranks.png", path = "jesslyn_plots/PDX_test")
```

* **CONCLUSIONS**: 
  - The logFC value associated with each gene is **identical** regardless of the statistical test used (correlation coefficient = 1). 
  - However, padj values differ significantly between statistical tests. The best correlation is between LR and the t.test, which has a correlation coefficient of **0.957**. 
