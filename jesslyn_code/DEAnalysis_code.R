...
DEAnalysis_code <- function(seurat_object, markers = NULL, group.by, group.1, group.2, graph = FALSE, oxphosGenes = NULL, utrGenes = NULL){
  ...
  all = EnhancedVolcano(DEgenes, lab=rownames(DEgenes), x='avg_logFC', y='p_val_adj', 
    ...
    title= paste(group.1, "vs", group.2, "DE Genes (lab all Genes)"), subtitle= "LogFC cutoff: 0.5, p cutoff: 0.05")
  ...
  if(!(is.null(oxphosGenes))){
    ...
    all_lab_oxphos = EnhancedVolcano(DEgenes, lab= rownames(DEgenes), x='avg_logFC', y='p_val_adj', 
        pCutoff = 0.05, FCcutoff = 0.5, col = c("black", "black", "black", "red2"), selectLab=oxphosGenes, 
        title=paste(group.1, "vs", group.2, "DE Genes (lab oxphos genes)"), subtitle= "LogFC cutoff: 0.5, p cutoff: 0.05")
    ...
    just_oxphos = EnhancedVolcano(DEgenes.oxphos,lab= rownames(DEgenes.oxphos), x='avg_logFC', y='p_val_adj', 
        pCutoff = 0.05, FCcutoff = 0.5, col = c("black", "black", "black", "red2"), 
        title=paste(group.1, "vs", group.2, "Oxphos DE Genes"), subtitle= "LogFC cutoff: 0.5, p cutoff: 0.05")
  ...
  if(!(is.null(utrGenes))){
    ...
    all_lab_utr = EnhancedVolcano(DEgenes, lab= rownames(DEgenes), x='avg_logFC', y='p_val_adj', 
        pCutoff = 0.05, FCcutoff = 0.5, col = c("black", "black", "black", "red2"), selectLab=utrGenes, 
        title=paste(group.1, "vs", group.2, "DE Genes (lab utr genes)"), subtitle= "LogFC cutoff: 0.5, p cutoff: 0.05")
    ...
    just_utr = EnhancedVolcano(DEgenes.utr,lab= rownames(DEgenes.utr), x='avg_logFC', y='p_val_adj', 
        pCutoff = 0.05, FCcutoff = 0.5, col = c("black", "black", "black", "red2"), 
        title=paste(group.1, "vs", group.2, "UTR DE Genes"), subtitle= "LogFC cutoff: 0.5, p cutoff: 0.05")
