...
signaturesData = read_csv("gene_lists/Ovarian_Signatures_combined.csv")
...
signatures <- colnames(signaturesData)
for (sig in signatures){
  cohort1 <- AddModuleScore(cohort1, features = list(select(signaturesData, sig)), name = sig)
}
```

```{r score cells with oxphos genes}
cohort1 <- AddModuleScore(cohort1, features = list(oxphosGenes), name = "oxphosGenes.Score")
```
