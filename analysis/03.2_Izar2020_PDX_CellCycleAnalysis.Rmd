...
for(i in 1:3){
  ...
  p <- ggplot(PDX@meta.data, aes(x= treatment.status , y = G2M.checkpoint43.centered, color = treatment.status)) +
  geom_quasirandom(groupOnX =TRUE) + 
  theme_bw() + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
  ...
  geom_boxplot(width = 0.10, position = position_dodge(0.9), alpha = 1, show.legend = F, color = "black", outlier.alpha = 0) + 
    geom_text(label = paste(sum(PDX$treatment.status == "vehicle"), "cells"), x = "vehicle", y = min(PDX[["G2M.checkpoint43.centered"]]) -0.03, show.legend = FALSE, color = "black") + 
    geom_text(label = paste(sum(PDX$treatment.status == "MRD"), "cells"), x = "MRD", y = min(PDX[["G2M.checkpoint43.centered"]]) - 0.03, show.legend = FALSE, color = "black") + 
    geom_text(label = paste(sum(PDX$treatment.status == "relapse"), "cells"), x = "relapse", y = min(PDX[["G2M.checkpoint43.centered"]]) - 0.03, show.legend = FALSE, color = "black") + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.format", step.increase = 0.06) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.signif", step.increase = 0.06, bracket.size = 0, vjust = 1.8)
...
for(i in 1:3){
  ...
  p <- ggplot(PDX@meta.data, aes(x= treatment.status , y = G2M.DNAdamage44.centered, color = treatment.status)) +
  geom_quasirandom(groupOnX =TRUE) + 
  theme_bw() + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
  ...
  geom_boxplot(width = 0.10, position = position_dodge(0.9), alpha = 1, show.legend = F, color = "black", outlier.alpha = 0) + 
    geom_text(label = paste(sum(PDX$treatment.status == "vehicle"), "cells"), x = "vehicle", y = min(PDX[["G2M.DNAdamage44.centered"]]) -0.03, show.legend = FALSE, color = "black") + 
    geom_text(label = paste(sum(PDX$treatment.status == "MRD"), "cells"), x = "MRD", y = min(PDX[["G2M.DNAdamage44.centered"]]) - 0.03, show.legend = FALSE, color = "black") + 
    geom_text(label = paste(sum(PDX$treatment.status == "relapse"), "cells"), x = "relapse", y = min(PDX[["G2M.DNAdamage44.centered"]]) - 0.03, show.legend = FALSE, color = "black") + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.format", step.increase = 0.06) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.signif", step.increase = 0.06, bracket.size = 0, vjust = 1.8)
...
for(i in 1:3){
  ...
  p <- ggplot(PDX@meta.data, aes(x= treatment.status , y = G1S.transcription42.centered, color = treatment.status)) +
  geom_quasirandom(groupOnX =TRUE) + 
  theme_bw() + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
  ...
  geom_boxplot(width = 0.10, position = position_dodge(0.9), alpha = 1, show.legend = F, color = "black", outlier.alpha = 0) + 
    geom_text(label = paste(sum(PDX$treatment.status == "vehicle"), "cells"), x = "vehicle", y = min(PDX[["G1S.transcription42.centered"]]) -0.03, show.legend = FALSE, color = "black") + 
    geom_text(label = paste(sum(PDX$treatment.status == "MRD"), "cells"), x = "MRD", y = min(PDX[["G1S.transcription42.centered"]]) - 0.03, show.legend = FALSE, color = "black") + 
    geom_text(label = paste(sum(PDX$treatment.status == "relapse"), "cells"), x = "relapse", y = min(PDX[["G1S.transcription42.centered"]]) - 0.03, show.legend = FALSE, color = "black") + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.format", step.increase = 0.06) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.signif", step.increase = 0.06, bracket.size = 0, vjust = 1.8)
...
for(i in 1:3){
  ...
  feature.found <- sum(hallmark.list[["G1S"]] %in% rownames(PDX))
  feature.length <- length(hallmark.list[["G1S"]])
  feature.pFound <- round((feature.found / feature.length)*100, 2)
  numCells <- nrow(PDX@meta.data)
  
  p <- ggplot(PDX@meta.data, aes(x= treatment.status , y = G1S41.centered, color = treatment.status)) +
  geom_quasirandom(groupOnX =TRUE) + 
  theme_bw() + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
    labs(title = glue("{PDX.name} G1/S Specific Transcription expression across treatment"), x = PDX.name, subtitle = glue("{numCells} Malignant Cells, {feature.found} out of {feature.length} G1/S genes found ({feature.pFound}%)")) + 
    theme(plot.title = element_text(size = 10), plot.subtitle = element_text(size = 8)) +
  geom_boxplot(width = 0.10, position = position_dodge(0.9), alpha = 1, show.legend = F, color = "black", outlier.alpha = 0) + 
    geom_text(label = paste(sum(PDX$treatment.status == "vehicle"), "cells"), x = "vehicle", y = min(PDX[["G1S41.centered"]]) -0.03, show.legend = FALSE, color = "black") + 
    geom_text(label = paste(sum(PDX$treatment.status == "MRD"), "cells"), x = "MRD", y = min(PDX[["G1S41.centered"]]) - 0.03, show.legend = FALSE, color = "black") + 
    geom_text(label = paste(sum(PDX$treatment.status == "relapse"), "cells"), x = "relapse", y = min(PDX[["G1S41.centered"]]) - 0.03, show.legend = FALSE, color = "black") + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.format", step.increase = 0.06) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.signif", step.increase = 0.06, bracket.size = 0, vjust = 1.8)
...
PDXs <- c(DF20.vehicle, DF20.MRD, DF20.relapse, DF101.vehicle, DF101.MRD, DF101.relapse, DF68.vehicle, DF68.MRD, DF68.relapse)
PDX.names <- c("DF20 Vehicle", "DF20 MRD", "DF20 Relapse", "DF101 Vehicle", "DF101 MRD", "DF101 Relapse", "DF68 Vehicle", "DF68 MRD", "DF68 Relapse")
plots <- vector("list", length = length(PDX.names))
names(plots) <- PDX.names

for(i in 1:length(PDXs)){
  PDX <- PDXs[[i]]
  PDX.name <- PDX.names[[i]]
  
  scores <- PDX@meta.data %>% 
  ...
  p <- ggplot(scores, aes(x = G1S41.centered, y = G2M.Score.centered, colour = Cycling)) + 
    geom_point() + 
    theme_bw() +
    labs(title = glue("PDX {PDX.name} Cell Cycle State"), subtitle = glue("N = {numCells}, {cycling}% cycling"), x = "G1/S Score", y = "G2/M Score") + 
...
plots[["DF20 Vehicle"]] + plots[["DF20 MRD"]] + plots[["DF20 Relapse"]] + plot_layout(guides = "collect")
plots[["DF101 Vehicle"]] + plots[["DF101 MRD"]] + plots[["DF101 Relapse"]] + plot_layout(guides = "collect")
plots[["DF68 Vehicle"]] + plots[["DF68 MRD"]] + plots[["DF68 Relapse"]] + plot_layout(guides = "collect")
...
for(i in 1:length(PDXs)){
  ...
  scores <- PDX@meta.data %>% 
    select(G1S41.centered, G2M.Score.centered, UNUPDATED.OXPHOS37.centered)
  cycling <- vector(mode = "character", length = nrow(scores))
  cycling[(scores$G1S41.centered < 1 | scores$G2M.Score.centered < 1)] <- "Non-Cycling"
  cycling[(scores$G1S41.centered > 1 | scores$G2M.Score.centered > 1)] <- "Intermediate"
  cycling[(scores$G1S41.centered > 2 | scores$G2M.Score.centered > 2)] <- "Cycling"
  ...
  scores <- scores %>% mutate("Cycling" = cycling)
  scores <- scores %>% mutate("highOXPHOS" = scores$UNUPDATED.OXPHOS37.centered > 0.2)
  ...
  p <- ggplot(scores, aes(x = G1S41.centered, y = G2M.Score.centered, colour = highOXPHOS)) + 
    geom_point() + 
    theme_bw() +
    labs(title = glue("PDX {PDX.name} Cell Cycle State and OXPHOS score"), subtitle = glue("N = {numCells}, {cycling}% cycling"), x = "G1/S Score", y = "G2/M Score", colour = "high OXPHOS (> 0.2)") + 
...
Does not seem to have any correlation between OXPHOS expression and cell cycle phase since some cycling cells also have high OXPHOS scores. Since we have a lot more noncycling cells, the fact that most of the cells that have a high OXPHOS score are also noncycling does not signify that a noncycling cell phase is associated with a higher OXPHOS score. It is very difficult to conclude anything because we have very little cells. 
