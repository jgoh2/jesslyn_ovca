...
for(i in 1:10){
  for(z in c("positive", "negative")){
    PC <- PCs[[i]]
    type <- z
    write_csv(DF20.pc.go1[[PC]][[z]], path = glue("jesslyn_plots/GO/PDX/UF/GOuf_DF20_{PC}_{type}.csv"))
    write_csv(DF101.pc.go1[[PC]][[z]], path = glue("jesslyn_plots/GO/PDX/UF/GOuf_DF101_{PC}_{type}.csv"))
    write_csv(DF68.pc.go1[[PC]][[z]], path = glue("jesslyn_plots/GO/PDX/UF/GOuf_DF68_{PC}_{type}.csv"))
  }
}
...
for(i in 1:10){
  ...
  ggsave(plot = p, filename = glue("PDXuf_{PC}.png"), path = "GO_plots/PDX/UF", width = 30, height = 20)
  
}

```

**PART 2: PC LOADINGS FROM PC1-PC5 OPTION ii (filter out insignificant loadings first)** 
...
for(i in 1:10){
  for(z in c("positive", "negative")){
    PC <- PCs[[i]]
    type <- z
    write_csv(DF20.pc.go2[[PC]][[z]], path = glue("jesslyn_plots/GO/PDX/F/GOf_DF20_{PC}_{type}.csv"))
    write_csv(DF101.pc.go2[[PC]][[z]], path = glue("jesslyn_plots/GO/PDX/F/GOf_DF101_{PC}_{type}.csv"))
    write_csv(DF68.pc.go2[[PC]][[z]], path = glue("jesslyn_plots/GO/PDX/F/GOf_DF68_{PC}_{type}.csv"))
  }
}
...
for(i in 1:10){
  for(z in c("positive", "negative")){
    ...
    plot.DF20 <- ggplot(data.DF20, aes(x= reorder(Term, -padj_BY_elim), y= (Significant/Annotated), fill = padj_BY_elim)) +
              ...
              labs(title = glue("DF20 {PC} {z} significant GO terms"), x = "Biological Process", y = "% Significant/Annotated") +
              coord_flip()
    ggsave(plot = plot.DF20, filename = glue("DF20f_{PC}_{z}.png"), path = "jesslyn_plots/GO/PDX/F", width = 15)
    ...
    plot.DF101 <- ggplot(data.DF101, aes(x= reorder(Term, -padj_BY_elim), y= (Significant/Annotated), fill = padj_BY_elim)) +
              ...
              labs(title = glue("DF101 {PC} {z} significant GO terms"), x = "Biological Process", y = "% Significant/Annotated") +
              coord_flip()
    ggsave(plot = plot.DF101, filename = glue("DF101f_{PC}_{z}.png"), path = "jesslyn_plots/GO/PDX/F", width = 15)
    ...
    plot.DF68 <- ggplot(data.DF68, aes(x= reorder(Term, -padj_BY_elim), y= (Significant/Annotated), fill = padj_BY_elim)) +
              ...
              labs(title = glue("DF68 {PC} {z} significant GO terms"), x = "Biological Process", y = "% Significant/Annotated") +
              coord_flip()
    ggsave(plot = plot.DF20, filename = glue("DF68f_{PC}_{z}.png"), path = "jesslyn_plots/GO/PDX/F", width = 15)
  }
}
