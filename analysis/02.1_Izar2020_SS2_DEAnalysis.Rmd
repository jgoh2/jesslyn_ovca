...
for(i in 1:10){
  for(z in c("positive", "negative")){
    PC <- PCs[[i]]
    type <- z
    write_csv(p8.pc.go1[[PC]][[z]], path = glue("jesslyn_plots/GO/SS2/UF/GOuf_P8_{PC}_{type}.csv"))
    write_csv(p9.pc.go1[[PC]][[z]], path = glue("jesslyn_plots/GO/SS2/UF/GOuf_P9_{PC}_{type}.csv"))
  }
}
...
for(i in 1:10){
  ...
  ggsave(plot = p, filename = glue("SS2uf_{PC}.png"), path = "GO_plots/SS2/UF", width = 30, height = 11)
...
```{r GO2, message=FALSE, warning = FALSE}
PCs <- c("PC_1", "PC_2", "PC_3", "PC_4", "PC_5", "PC_6", "PC_7", "PC_8", "PC_9", "PC_10")
...
for(i in 1:10){
  for(z in c("positive", "negative")){
    PC <- PCs[[i]]
    type <- z
    write_csv(p8.pc.go2[[PC]][[z]], path = glue("jesslyn_plots/GO/SS2/F/GOf_P8_{PC}_{type}.csv"))
    write_csv(p9.pc.go2[[PC]][[z]], path = glue("jesslyn_plots/GO/SS2/F/GOf_P9_{PC}_{type}.csv"))
  }
}
...
nadh.swarm.plots[["SS2 Patient 8"]] + nadh.swarm.plots[["SS2 Patient 9"]]
...
resp.swarm.plots[["SS2 Patient 8"]] + resp.swarm.plots[["SS2 Patient 9"]]
...
translation.swarm.plots[["SS2 Patient 8"]] + translation.swarm.plots[["SS2 Patient 9"]]
...
term.swarm.plots[["SS2 Patient 8"]] + term.swarm.plots[["SS2 Patient 9"]]
...
pro.swarm.plots[["SS2 Patient 8"]] + pro.swarm.plots[["SS2 Patient 9"]]
...
```{r PCAGO, message = FALSE, warning = FALSE, fig.align = "center"}
...
for(i in 1:2){
  ...
  plots[[1]] <- ggplot(PCs[[SS2.name]], aes(x = PC_3, y = PC_6, colour = sample)) + geom_point() + labs(title = glue("{SS2.name} by Sample (PC3 vs PC6)")) + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
...
nadh.pca.plots[["SS2 Patient 8"]][[1]] + nadh.pca.plots[["SS2 Patient 8"]][[2]] + nadh.pca.plots[["SS2 Patient 9"]][[1]] + nadh.pca.plots[["SS2 Patient 9"]][[2]]
...
resp.pca.plots[["SS2 Patient 8"]][[1]] + resp.pca.plots[["SS2 Patient 8"]][[2]] + resp.pca.plots[["SS2 Patient 9"]][[1]] + resp.pca.plots[["SS2 Patient 9"]][[2]]
...
trans.pca.plots[["SS2 Patient 8"]][[1]] + trans.pca.plots[["SS2 Patient 8"]][[2]] + trans.pca.plots[["SS2 Patient 9"]][[1]] + trans.pca.plots[["SS2 Patient 9"]][[2]]
...
term.pca.plots[["SS2 Patient 8"]][[1]] + term.pca.plots[["SS2 Patient 8"]][[2]] + term.pca.plots[["SS2 Patient 9"]][[1]] + term.pca.plots[["SS2 Patient 9"]][[2]]
...
for(i in 1:2){
  ...
  plots[[1]] <- ggplot(PCs[[SS2.name]], aes(x = PC_3, y = PC_4, colour = sample)) + geom_point() + labs(title = glue("{SS2.name} by Sample (PC3 vs PC4)")) + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
    theme(plot.title = element_text(size = 10))
...
pro.pca.plots[["SS2 Patient 8"]][[1]] + pro.pca.plots[["SS2 Patient 8"]][[2]] + pro.pca.plots[["SS2 Patient 9"]][[1]] + pro.pca.plots[["SS2 Patient 9"]][[2]]
