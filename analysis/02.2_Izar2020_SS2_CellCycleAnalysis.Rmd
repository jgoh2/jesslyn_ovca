...
nc <- c("G0G139.centered", "G1S.DNAdamage40.centered", "G2M.checkpoint43.centered", "G2M.DNAdamage44.centered")
...
for(i in 1:2){
  ...
  p <- ggplot(SS2@meta.data, aes(x= sample, y = G2M.checkpoint43.centered, color = sample)) +
  geom_quasirandom(groupOnX =TRUE) + 
  theme_bw() + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
    ...
    theme(plot.title = element_text(size =10), plot.subtitle = element_text(size = 8)) +
  geom_boxplot(width = 0.10, position = position_dodge(0.9), alpha = 1, show.legend = F, color = "black", outlier.alpha = 0) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].0")), "cells"), x = glue("{patient[[i]]}.0"), y = min(SS2$G2M.checkpoint43.centered) -0.03, color = "black", show.legend = FALSE) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].1")), "cells"), x = glue("{patient[[i]]}.1"), y = min(SS2$G2M.checkpoint43.centered) - 0.03, color = "black", show.legend = FALSE) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].2")), "cells"), x = glue("{patient[[i]]}.2"), y = min(SS2$G2M.checkpoint43.centered) - 0.03, color = "black", show.legend = FALSE) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.format", step.increase = 0.06) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.signif", step.increase = 0.06, bracket.size = 0, vjust = 1.8)
...
for(i in 1:2){
  ...
  p <- ggplot(SS2@meta.data, aes(x= sample, y = G2M.DNAdamage44.centered, color = sample)) +
  geom_quasirandom(groupOnX =TRUE) + 
  theme_bw() + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
    ...
    theme(plot.title = element_text(size =10), plot.subtitle = element_text(size = 8)) + 
  geom_boxplot(width = 0.10, position = position_dodge(0.9), alpha = 1, show.legend = F, color = "black", outlier.alpha = 0) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].0")), "cells"), x = glue("{patient[[i]]}.0"), y = min(SS2$G2M.DNAdamage44.centered) -0.03, color = "black", show.legend = FALSE) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].1")), "cells"), x = glue("{patient[[i]]}.1"), y = min(SS2$G2M.DNAdamage44.centered) - 0.03, color = "black", show.legend = FALSE) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].2")), "cells"), x = glue("{patient[[i]]}.2"), y = min(SS2$G2M.DNAdamage44.centered) - 0.03, color = "black", show.legend = FALSE) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.format", step.increase = 0.06) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.signif", step.increase = 0.06, bracket.size = 0, vjust = 1.8)
...
#VlnPlot -------------------------
cc <- c("G1S.transcription42.centered", "G1S41.centered", "S.Score.centered", "G2M.Score.centered")
cc.names <- c("G1/S Specific Transcription", "G1/S", "S Score", "G2M Score")
...
for(i in 1:2){
  ...
  p <- ggplot(SS2@meta.data, aes(x= sample, y = G1S.transcription42.centered, color = sample)) +
  geom_quasirandom(groupOnX =TRUE) + 
  theme_bw() + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
    ...
    theme(plot.title = element_text(size =10), plot.subtitle = element_text(size = 8)) + 
  geom_boxplot(width = 0.10, position = position_dodge(0.9), alpha = 1, show.legend = F, color = "black", outlier.alpha = 0) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].0")), "cells"), x = glue("{patient[[i]]}.0"), y = min(SS2$G1S.transcription42.centered) -0.03, color = "black", show.legend = FALSE) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].1")), "cells"), x = glue("{patient[[i]]}.1"), y = min(SS2$G1S.transcription42.centered) - 0.03, color = "black", show.legend = FALSE) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].2")), "cells"), x = glue("{patient[[i]]}.2"), y = min(SS2$G1S.transcription42.centered) - 0.03, color = "black", show.legend = FALSE) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.format", step.increase = 0.06) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.signif", step.increase = 0.06, bracket.size = 0, vjust = 1.8)
...
for(i in 1:2){
  ...
  feature.found <- sum(hallmark.list[["G1S"]] %in% rownames(SS2))
  feature.length <- length(hallmark.list[["G1S"]])
  feature.pFound <- round((feature.found / feature.length)*100, 2)
  numCells <- nrow(SS2@meta.data)
  
  p <- ggplot(SS2@meta.data, aes(x= sample, y = G1S41.centered, color = sample)) +
  geom_quasirandom(groupOnX =TRUE) + 
  theme_bw() + 
  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + 
    labs(title = glue("{SS2.name} Malignant Cells G1/S expression across sample"), x = SS2.name, subtitle = glue("{numCells} Malignant Cells, {feature.found} out of {feature.length} G1/S genes found ({feature.pFound}%)")) + 
    theme(plot.title = element_text(size =10), plot.subtitle = element_text(size = 8)) + 
  geom_boxplot(width = 0.10, position = position_dodge(0.9), alpha = 1, show.legend = F, color = "black", outlier.alpha = 0) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].0")), "cells"), x = glue("{patient[[i]]}.0"), y = min(SS2$G1S41.centered) -0.03, color = "black", show.legend = FALSE) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].1")), "cells"), x = glue("{patient[[i]]}.1"), y = min(SS2$G1S41.centered) - 0.03, color = "black", show.legend = FALSE) + 
    geom_text(label = paste(sum(str_detect(SS2$sample, "[:digit:].2")), "cells"), x = glue("{patient[[i]]}.2"), y = min(SS2$G1S41.centered) - 0.03, color = "black", show.legend = FALSE) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.format", step.increase = 0.06) + 
    stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.signif", step.increase = 0.06, bracket.size = 0, vjust = 1.8)
...
**3) G2M vs. G1/S Scores Per Treatment** 
We reconfirm our results by plotting G2M scores against G1/S Scores for each treatment condition

```{r cellcycle2, message = FALSE, warning= FALSE, fig.align = 'center', fig.width = 12, fig.height = 4}
SS2s <- c(SS2Malignant8.0, SS2Malignant8.1, SS2Malignant8.2, SS2Malignant9.0, SS2Malignant9.1, SS2Malignant9.2)
SS2.names <- c("8.0", "8.1", "8.2", "9.0", "9.1", "9.2")
plots <- vector("list", length = length(SS2.names))
names(plots) <- SS2.names

for(i in 1:length(SS2s)){
  SS2 <- SS2s[[i]]
  SS2.name <- SS2.names[[i]]
  
  scores <- SS2@meta.data %>% 
    select(G1S41.centered, G2M.Score.centered)
  cycling <- vector(mode = "character", length = nrow(scores))
  cycling[(scores$G1S41.centered < 1 | scores$G2M.Score.centered < 1)] <- "Non-Cycling"
  cycling[(scores$G1S41.centered > 1 | scores$G2M.Score.centered > 1)] <- "Intermediate"
  cycling[(scores$G1S41.centered > 2 | scores$G2M.Score.centered > 2)] <- "Cycling"
  scores <- scores %>% mutate("Cycling" = cycling)
  
  numCells <- nrow(scores)
  cycling <- round(sum(scores$Cycling == "Cycling") / numCells, 2)
  levels(scores$Cycling) <- c("Cycling", "Intermediate", "Non-Cycling")
  myColors <- c("#00AFBB", "#E7B800", "#FC4E07")
  names(myColors) <- levels(scores$Cycling)
  
  p <- ggplot(scores, aes(x = G1S41.centered, y = G2M.Score.centered, colour = Cycling)) + 
    geom_point() + 
    theme_bw() +
    labs(title = glue("SS2 Sample {SS2.name} Cell Cycle State"), subtitle = glue("N = {numCells}, {cycling}% cycling"), x = "G1/S Score", y = "G2/M Score") + 
  scale_color_manual(name =  "Cycling", values = myColors) + 
    theme(plot.title = element_text(size = 10), plot.subtitle = element_text(size = 8))
  
  plots[[SS2.name]] <- p
}

plots[["8.0"]] + plots[["8.1"]] + plots[["8.2"]] + plot_layout(guides = "collect")
plots[["9.0"]] + plots[["9.1"]] + plots[["9.2"]] + plot_layout(guides = "collect")

```

We confirm that there does not seem to have a significant correlation between cell cycle phase and treatment condition. 
...
```{r cellcycle3, message=FALSE, warning=FALSE, fig.align='center', fig.width=10, fig.height = 8}
SS2s <- c(SS2Malignant.8, SS2Malignant.9)
SS2.names <- c("SS2 Patient 8", "SS2 Patient 9")
...
We reconfirm our results by plotting G2M vs. G1/S Scores, with points colored by high OXPHOS scores (> 2). 

```{r cellcycle4, message = FALSE, warning= FALSE, fig.align = 'center', fig.width = 12, fig.height = 4}
SS2s <- c(SS2Malignant8.0, SS2Malignant8.1, SS2Malignant8.2, SS2Malignant9.0, SS2Malignant9.1, SS2Malignant9.2)
SS2.names <- c("8.0", "8.1", "8.2", "9.0", "9.1", "9.2")
plots <- vector("list", length = length(SS2.names))
names(plots) <- SS2.names
...
for(i in 1:length(SS2s)){
  ...
  scores <- SS2@meta.data %>% 
    select(G1S41.centered, G2M.Score.centered, GO.OXPHOS35.centered)
  cycling <- vector(mode = "character", length = nrow(scores))
  cycling[(scores$G1S41.centered < 1 | scores$G2M.Score.centered < 1)] <- "Non-Cycling"
  cycling[(scores$G1S41.centered > 1 | scores$G2M.Score.centered > 1)] <- "Intermediate"
  cycling[(scores$G1S41.centered > 2 | scores$G2M.Score.centered > 2)] <- "Cycling"
  ...
  scores <- scores %>% mutate("Cycling" = cycling)
  scores <- scores %>% mutate("highOXPHOS" = scores$GO.OXPHOS35.centered > 0.2)
  ...
  p <- ggplot(scores, aes(x = G1S41.centered, y = G2M.Score.centered, colour = highOXPHOS)) + 
    geom_point() + 
    theme_bw() +
    labs(title = glue("SS2 Sample {SS2.name} Cell Cycle State and OXPHOS score"), subtitle = glue("N = {numCells}, {cycling}% cycling"), x = "G1/S Score", y = "G2/M Score", colour = "high OXPHOS ( > 0.2)") + 
...
Does not seem to have any correlation between OXPHOS expression and cell cycle phase since some cycling cells also have high OXPHOS scores. Again,it is very difficult to conclude anything because we have very little cells. 
