...
wilcox.volcano <- wilcox.volcano + theme(plot.title = element_text(size = 10), plot.subtitle = element_text(size = 8))

wilcox.FCrank.plot <- wilcox.FCrank.plot + theme(plot.title = element_text(size = 10), plot.subtitle = element_text(size = 8))
...
**Conclusion**: Although ranking genes by AUC seems like the best method, the drastic difference in logFC and padj values we get from the **wilcoxauc** vs. **wilcox** statistical tests is a concerning factor, and we're not sure which metric to trust.
