...
ovca_10x = AddModuleScore(ovca_10x, features = ovca_sigs, name = names(ovca_sigs), nbin = 50, search = T)
names(ovca_10x@meta.data)[(col+1):ncol(ovca_10x@meta.data)] = names(ovca_sigs)

col = ncol(ovca_SS2@meta.data)
ovca_SS2 = AddModuleScore(ovca_SS2, features = ovca_sigs, name = names(ovca_sigs), nbin = 50, search = T)
names(ovca_SS2@meta.data)[(col+1):ncol(ovca_SS2@meta.data)] = names(ovca_sigs)

col = ncol(ovca_PDX@meta.data)
ovca_PDX = AddModuleScore(ovca_PDX, features = ovca_sigs, name = names(ovca_sigs), nbin = 50, search = T)
names(ovca_PDX@meta.data)[(col+1):ncol(ovca_PDX@meta.data)] = names(ovca_sigs)
```

### Generate summary table for each dataset

 - cells per patient 
 - cells per patient per cell.type
 - malignant cells per patient per time
 - malignant cells per patient per treatment.status
