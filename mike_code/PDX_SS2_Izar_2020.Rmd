...
plots = lapply(feats, function(x) {
      plots[[x]] = plots[[x]] + 
        ...
        labs(x = "model") + theme(plot.title = element_text(size = 10))
      })
names(plots) = feats
...
wrap_plots(plots, ncol = 1, guides = "collect") %>% ggsave(glue('{fig.dir}/PDX_dotplots.png'), width = 7, height = 14, plot = .)
...
group2 = unique(ovca_PDX$treatment.status)
